...
class EasyDBSqlite
{
    ...
    /**
     * fast hack for SQLite specific on conflict replace
     */
    public function replace(string $table, array $map): int
    {
        if (!empty($map)) {
            if (!$this->is1DArray($map)) {
                throw new Issues\MustBeOneDimensionalArray(
	...
	$queryString = str_replace('INSERT INTO ','REPLACE INTO ',$queryString);
     ...
     * fast hack for SQLite specific on conflict replace
     */
    public function replaceGet(string $table, array $map, string $field)
    {
        if (empty($map)) {
            throw new \Exception('An empty array is not allowed for insertGet()');
        }
        if ($this->replace($table, $map) < 1) {
            throw new \Exception('Insert failed');
        }
        $post = [];
        $params = [];
    ...
    /**
     * fast hack for sqlite replace
     */ 
    public function replaceMany(string $table, array $maps): int
    {
        if (count($maps) < 1) {
            throw new \InvalidArgumentException(
                'Argument 2 passed to ' .
    ...
    public function buildInsertQueryBoolSafe(
        ...
        */
        ...
        $query = \sprintf(
            ...
            (false === $duplicates_mode ? ' OR IGNORE ' : ' '),
            $this->escapeIdentifier($table),
            \implode(', ', $columns),
            \implode(', ', $placeholders),
            (
